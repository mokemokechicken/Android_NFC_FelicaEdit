...
public class FelicaTag extends NfcTag{
    ...
    public FelicaTag(Parcelable tag, byte[] id) {
        super(tag, id);
        this.idm = new IDm(id);
    }
     ...
     * @param systemCode Pollingを行う対象の systemCode
    ...
    public byte[] doSearchServiceCode(IDm idm, int index) throws NfcException {
        CommandPacket reqServiceCode =
            new CommandPacket(COMMAND_SEARCH_SERVICECODE, idm, new byte[]{(byte)(index & 0xff), (byte)(index >> 8)});
     ...
     */
    public byte[] readWithoutEncryption(int serviceCode, byte addr) throws NfcException {
        return readWithoutEncryption(new ServiceCode(serviceCode), addr);
    }

    /**
     * 認証不要なサービスコードのデータを読み取ります。
     * 本来は、複数のブロックを同時に読めます。 
     * JIS_X_6319_4 を見ると複数ブロックにアクセスする方法がわかります。
     * @param serviceCode データを読み取るサービスコード
     * @param addr 何ブロック目を読むか。 0～N
     * @return 読み取ったブロックのByte列を返します。読み取りステータスが正常でなければnullを返します。
     * @throws NfcException
     */
    public byte[] readWithoutEncryption(ServiceCode serviceCode, int addr) throws NfcException {
    ...
    /**
     * 本来は複数ブロックやサービスに同時に書き込めますが、この実装は１ブロックだけです。
     * JIS_X_6319_4 を見ると複数ブロックに書き込む方法がわかります。
     ...
     * @throws NfcException
     */
    public int writeWithoutEncryption(ServiceCode serviceCode, int addr, byte[] buff) throws NfcException {
     ...
     */
    public int writeWithoutEncryption(int serviceCode, byte addr, byte[] buff) throws NfcException {
